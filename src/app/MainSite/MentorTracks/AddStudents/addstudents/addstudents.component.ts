...
export class AddstudentsComponent implements OnInit {
  ...
  constructor(private employeeService: EmployeeService, private SpinnerService: NgxSpinnerService, private bookService: BookService, private toaster: ToastrService
      ...
      if (form.valid && !this.duplicatetrackname && !this.duplicatebook) {
        this.bookService.AddTrackCategory(this.BookModel).subscribe(res => {
          if (res.code == 200) {
            ...
            if (this.superadmin) {
              this._router.navigateByUrl('/Main/mentortracklist')
            }
            else {
              this._router.navigateByUrl('/Main/mentortracklist')
            }
  ...
  AddAsCustom(form: NgForm) {
    ...
    this.bookService.GetTrackList(this.userid, this.companyid, this.individual).subscribe(res => {
      ...
      if (form.valid && !this.duplicatetrackname && !this.duplicatebook && this.isStudentSelected) {
